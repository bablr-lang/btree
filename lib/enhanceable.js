...
export const buildModule = (NODE_SIZE = defaultNodeSize, buildStats, type = Symbol.for('List')) => {
  ...
  const treeFromValues = (values) => {
    ...
    if (values.length <= NODE_SIZE) {
      ...
      let firstType = isNode(firstValue) ? getType(firstValue) : null;
      if (firstType && firstType !== type) throw new Error();
