...
export const buildModule = (NODE_SIZE = defaultNodeSize, buildStats, type = 'List') => {
  ...
  const treeFromValues = (values) => {
    ...
    if (values.length <= NODE_SIZE) {
      ...
      return freeze([countValues(values), freeze(values), stats, type]);
    } else {
      throw new Error();
    }
  };
  ...
  const validateValues = (values) => {
    ...
    let isNode = !isLeaf(values[0]);
    for (let value of values) {
      if (typeof value === 'number') {
        throw new Error('invalid btree: number as value');
      } else if (isNode && nodeCollapses(value)) {
        throw new Error('invalid btree: uncollapsed node');
      } else if (isNode && value == null) {
