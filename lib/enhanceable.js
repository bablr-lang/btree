...
export const buildModule = (NODE_SIZE = defaultNodeSize, buildSums, type = Symbol.for('List')) => {
  ...
  const treeFromValues = (values, depth) => {
    ...
    if (isArray(values) && values.length <= NODE_SIZE) {
      let sums = buildSums ? buildSums(values, depth_) : null;
  ...
  const concat = (first, second) => {
    ...
    if (firstHeight === secondHeight) {
      ...
      if (!nodeCollapses(pushout)) {
        ...
        let targetPath = findPath(targetIndex, tree, freezeRecord({ depth }));
  ...
  const addAt = (idx, value, tree) => {
    ...
    let path = findPath(idx, tree, freezeRecord({ depth: getDepth(tree) - getDepth(value) }));
  ...
  const findPath = (idx, tree, options = freezeRecord({ depth: Infinity })) => {
    if (idx == null) throw new Error();
    if (tree && !isArray(tree)) throw new Error();
    if (options && !isRecord(options)) throw new Error();
    ...
    if (isArray(idx)) {
      if (idx.length > Math.min(options.depth, treeDepth)) return null;
    ...
    stack: while (node) {
      ...
      for (
        ...
        let value = values[i];
        if (isNode(value) && path.length + 1 < Math.min(options.depth, treeDepth)) {
          candidateNode = value;
