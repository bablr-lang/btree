...
export const buildModule = (NODE_SIZE = defaultNodeSize, buildStats) => {
  ...
  const concat = (first, second) => {
    ...
    if (firstHeight === secondHeight) {
      ...
      if (!nodeCollapses(pushout)) {
        ...
        let depth = Math.abs(firstHeight - secondHeight);
        let targetIndex = firstHeight > secondHeight ? getSize(tree) - 1 : 0;
        let targetPath = findPath(targetIndex, tree, depth);
        let targetNode = getValues(targetPath.value.node)[targetPath.value.index];
        if (getValues(pushout).length + getValues(targetNode).length <= NODE_SIZE) {
          return replaceAt(
            targetIndex,
            tree,
            treeFromValues([...getValues(pushout), ...getValues(targetNode)]),
          );
        } else {
          let { leftValues, rightValues } = splitValues([
            ...getValues(pushout),
            ...getValues(targetNode),
          ]);

          if (firstHeight > secondHeight) {
            return addAt(
              targetIndex + 1,
              replaceAt(targetIndex, tree, treeFromValues([...getValues(pushout), ...leftValues])),
              rightValues,
            );
          } else {
            return addAt(
              targetIndex,
              replaceAt(targetIndex, tree, treeFromValues([...getValues(pushout), ...rightValues])),
              leftValues,
            );
          }
        }
  ...
  const addAt = (idx, tree, value) => {
    ...
    let path = findPath(idx, tree, getHeight(tree) - getHeight(value));
    ...
    stack: while (node) {
      ...
      for (
        ...
        if (isArray(value) && path.size < depth - 1) {
          ...
          } else if (backwards ? nextIndex + 1 <= targetIdx : nextIndex - 1 >= targetIdx) {
