...
export const buildModule = (NODE_SIZE = defaultNodeSize, buildStats) => {
  ...
  const treeFromValues = (values) => {
    ...
    if (values.length <= NODE_SIZE) {
      if (buildStats) {
        return freeze([countValues(values), freeze(values), buildStats(values)]);
      } else {
        return freeze([countValues(values), freeze(values)]);
      }
    } else {
      throw new Error();
    }
