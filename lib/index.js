...
export const add = (tree, value) => {
  ...
  path = path.replace({ node, index: index + 1 });
  ...
  for (;;) {
    ...
    node = setValuesAt(index, node, popped.value.node);
    values = getValues(node);
    path = path.replace({ node, index });
  }
};

export const collapses = (size) => {
...
export const nodeCanDonate = (node) => {
  return collapses(getValues(node).length - 1);
};

export const pop = (tree) => {
  let path = findPath(-1, tree);
...
export const setValues = (node, values) => {
...
export const findPath = (idx, tree) => {
  ...
  stack: while (node) {
    ...
    if (isLeafNode(node)) {
      ...
      return path.push({ index: targetIdx - startIdx, node });
    } else {
      const values = node[1];
...
export const replaceAt = (idx, tree, value) => {
